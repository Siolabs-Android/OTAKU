...
public class CAforFriendRequest extends ArrayAdapter {
	...
	int userId;
	ArrayList<User_table> requests;
	ArrayList<String> s;
	FriendList fl;

	public CAforFriendRequest(Context ctx, ArrayList<User_table> requests,
			ArrayList<String> s) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final Button accept = (Button) row.findViewById(R.id.button2);
		final Button reject = (Button) row.findViewById(R.id.button1);
		accept.setTag((Integer) position);
		if (position < requests.size()) {
			if (requests.get(position).getDp() != null)
				iv1.setImageBitmap(BitmapFactory.decodeFile(requests.get(
						position).getDp()));
			...
			accept.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					int p = (Integer) accept.getTag();
					fd.acceptFriend(requests.get(p).get_id());
					FriendList.addFriend(p);
					requests.remove(p);
					s.remove(p);
